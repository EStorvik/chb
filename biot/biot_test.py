...
# Flow
F_f = (p-p_old)/compressibility*q*dx+alpha*div(u_n-u_old)*q*dx+dt*permeability*dot(grad(p),grad(q))*dx+stabilization*(p-p_prev)*q*dx - dt*S_f*q*dx
A_f, L_f = df.lhs(F_f), df.rhs(F_f)

# Elasticity 
F_e = 2*lame_mu*inner(sym(grad(u)),sym(grad(v)))*dx+lame_lambda*div(u)*div(v)*dx-alpha*p_n*div(v)*dx - dot(f, v)*dx
A_e, L_e = df.lhs(F_e), df.rhs(F_e)

output_file_p = df.File("/home/erlend/src/fenics/output/biot/manufactured_solution_pressure.pvd", "compressed")
output_file_u = df.File("/home/erlend/src/fenics/output/biot/manufactured_solution_displacement.pvd", "compressed")
