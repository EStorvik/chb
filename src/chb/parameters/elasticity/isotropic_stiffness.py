...
class IsotropicStiffnessTensor:
    ...
    def stress(self, strain, pf):
        return (
            self.lame_lambda(pf) * ufl.tr(strain) * ufl.Identity(self.dim)
            + 2 * self.lame_mu(pf) * strain
        )

    def stress_prime(self, strain, pf):
        return (
            self.lame_lambda_prime(pf) * ufl.tr(strain) * ufl.Identity(self.dim)
            + 2 * self.lame_mu_prime(pf) * strain
        )

    def lame_lambda(self, pf):
        return self.lame_lambda_0 + self.interpolator(pf) * (
            self.lame_lambda_1 - self.lame_lambda_0
        )
    ...
    def lame_mu(self, pf):
        return self.lame_mu_0 + self.interpolator(pf) * (
            self.lame_mu_1 - self.lame_mu_0
        )
