...
manufsol = chb.CHBManufacturedSolution(doublewell=doublewell, mobility = mobility, gamma=gamma, ell=ell, M=M, alpha=alpha, kappa=k, stiffness=stiffness, swelling=swelling)
...
# Function spaces
ME = df.MixedElement([P1, P1, P2V, P2V, P0])
...
F_mu = (
    ...
    - gamma/ell * (doublewell.prime(pf_prev) + doublewell.doubleprime(pf_prev) * (pf - pf_prev))
    * eta_mu
    * dx
...
for i in range(num_time_steps):
        ...
        for j in range(max_iter):
            ...
            print(f"Norm at time step {i} iteration {j}: {sqrt(df.assemble((pf_n - pf_prev)**2*dx))}")
