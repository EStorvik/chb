#!/usr/bin/env python3
"""
Automated script to run parametric study over gamma values for all semi_implicit CHB simulations.
...
def main():
    ...
    print(f"Total simulations: {len(GAMMA_VALUES)} × {len(SIMULATION_SCRIPTS)} = {len(GAMMA_VALUES) * len(SIMULATION_SCRIPTS)}")
    print("=" * 70)
    ...
    for i, gamma in enumerate(GAMMA_VALUES, 1):
        ...
        print(f"  Summary: {gamma_successes}/{len(SIMULATION_SCRIPTS)} simulations successful")
