...
def create_performance_plots(results: Dict[str, Dict[float, Dict]]) -> None:
    ...
    fig.suptitle(
        f"CHB Swelling Parameter Study - Performance Analysis (γ = {FIXED_GAMMA})",
        fontsize=16,
        fontweight="bold",
    )
    ...
    for i, script_name in enumerate(SCRIPT_NAMES):
        if script_name in results:
            ...
            for swelling in swelling_vals:
                if swelling in results[script_name]:
                    iterations.append(
                        results[script_name][swelling]["total_iterations"]
                    )
                else:
                    iterations.append(0)
    ...
    ax3.set_title(
        "Average Iterations per Time Step vs Swelling Parameter", fontweight="bold"
    )
    for i, script_name in enumerate(SCRIPT_NAMES):
        if script_name in results:
            avg_iters = []
            for swelling in swelling_vals:
                if swelling in results[script_name]:
                    avg_iters.append(
                        results[script_name][swelling]["avg_iterations_per_step"]
                    )
                else:
                    avg_iters.append(0)
...
def print_summary_stats(results: Dict[str, Dict[float, Dict]]) -> None:
    ...
    for script_name in SCRIPT_NAMES:
        if script_name in results and results[script_name]:
            ...
            total_times = [
                results[script_name][swelling]["total_time"]
            ...
            if total_times and total_iters:
                ...
                print(
                    f"   Available swelling values: {sorted(results[script_name].keys())}"
                )
...
def main():
    ...
    if not OUTPUT_DIR.exists():
        ...
        print(
            "Please run the swelling parameter study first using run_swelling_study.py"
        )
        return
    ...
    print(
        f"  📊 Performance plots: {OUTPUT_DIR / 'swelling_study_performance_analysis.png'}"
    )
