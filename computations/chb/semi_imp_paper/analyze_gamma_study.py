...
def create_performance_plots(results: Dict[str, Dict[float, Dict]]) -> None:
    ...
    fig.suptitle('CHB Gamma Parameter Study - Performance Analysis', fontsize=16, fontweight='bold')
    ...
    for i, script_name in enumerate(SCRIPT_NAMES):
        if script_name in results:
            ...
            for gamma in gamma_vals:
                if gamma in results[script_name]:
                    avg_iters.append(results[script_name][gamma]['avg_iterations_per_step'])
                else:
                    avg_iters.append(0)
...
def print_summary_stats(results: Dict[str, Dict[float, Dict]]) -> None:
    ...
    for script_name in SCRIPT_NAMES:
        if script_name in results and results[script_name]:
            ...
            total_times = [results[script_name][gamma]['total_time'] 
                          for gamma in results[script_name]]
            total_iters = [results[script_name][gamma]['total_iterations'] 
                          for gamma in results[script_name]]
            ...
            if total_times and total_iters:
                ...
                print(f"   Available gamma values: {sorted(results[script_name].keys())}")
...
def main():
    ...
    print(f"  📊 Performance plots: {OUTPUT_DIR / 'gamma_study_performance_analysis.png'}")
