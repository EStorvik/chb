...
def splitting_ch_fs_imp(parameters):
    ...
    F_theta = (
        ...
        + L * inner(p - p_prev, eta_theta) * dx
        + dt * permeability * inner(grad(p), grad(eta_theta)) * dx
    )

    F_p = (
        inner(p, eta_p) * dx
        - inner(compressibility(pf) * (theta - alpha(pf) * div(u_n)), eta_p) * dx
    )
