...
MEf = mixed_element([P1, P1])

# Function spaces
Vch = functionspace(msh, MEch)
Ve = functionspace(msh, P1U)
Vf = functionspace(msh, MEf)
...
u = TrialFunction(Ve)
eta_u = TestFunction(Ve)
u_n = Function(Ve)

xiF = TrialFunction(Vf)
etaF = TestFunction(Vf)
theta, p = split(xiF)
eta_theta, eta_p = split(etaF)
xiF_n = Function(Vf)
...
u_n.interpolate(lambda x: np.zeros((2, x.shape[1])))
xiF_n.sub(0).interpolate(lambda x: np.zeros((1, x.shape[1])))
xiF_n.sub(1).interpolate(lambda x: np.zeros((1, x.shape[1])))
...
dofs_u = locate_dofs_topological(Ve, msh.topology.dim - 1, facets)
...
F_mu = (
    ...
    * dx
    - (
        inner(
            0.5 *
            inner(
                stiffness_tensor.stress_prime(strain=sym(grad(u_prev)) - swelling(pf), pf=pf),
    ...
    * dx
    - (
        inner(
            0.5 * compressibility.prime(pf) * (theta_prev - alpha(pf) * div(u_prev))**2
            - alpha.prime(pf) * compressibility(pf) * (theta_prev - alpha(pf) * div(u_prev)) * div(u_prev),
            eta_mu
        )
        * dx
    )
...
F_u = (
    inner(
        ...
        - alpha(pf) * compressibility(pf) * (theta_prev - alpha(pf) * div(u)) * Identity(2),
        sym(grad(eta_u))
    )
    * dx
)
...
F_theta = (
    inner(theta - theta_old, eta_theta) * dx +L*inner(p-p_prev, eta_theta)*dx + dt * permeability * inner(grad(p), grad(eta_theta)) * dx
)

F_p = (
    inner(p, eta_p) * dx  - inner(compressibility(pf) * (theta - alpha(pf) * div(u_n)), eta_p) * dx              
) 
...
au = lhs(F_u)
lu = rhs(F_u)
problemu = LinearProblem(au, lu, bcs=[bc_u]) #bcs=[bc_u, bc_p_left, bc_p_right], bcs=[bc_u]

# Flow
af = lhs(Ff)
lf = rhs(Ff)
problemf = LinearProblem(af, lf, bcs = [])
...
for i in range(num_time_steps):
    ...
    xiF_old.x.array[:] = xiF_n.x.array
    xiF_old.x.scatter_forward()
    ...
    for j in range(max_iter_split):
        ...
        xiF_prev.x.array[:] = xiF_n.x.array
        xiF_prev.x.scatter_forward()
        # Solve the non-linear problems
        nCH, convergedCH = solverCH.solve(xiCH)

        for k in range(max_iter_split):
            u_prev.x.array[:] = u_n.x.array
            u_prev.x.scatter_forward()
            xiF_prev.x.array[:] = xiF_n.x.array
            xiF_prev.x.scatter_forward()
            theta_prev, p_prev = xiF_prev.split()
            u_n = problemu.solve()
            xiF_n = problemf.solve()
            theta_n, p_n = xiF_n.split()
            increment_fs = chb.util.l2norm_2(u_n-u_prev, p_n-p_prev)
            print(f"Fixed_stress iteration increment error: {increment_fs}")
            if increment_fs < tol_split:
                break
...
log_filenamepath = "../output/log/chb_splitting_ch_fixedstress_imp.xlsx"
